...
public class MapController : MonoBehaviour
{
    ...
    public GameObject AddStreetPopUp;
    ...
    public float usersRadius = 25f;
    
    // List of Vecotr3s
    public static List<Vector3> listVectors;
    private List<Vector3> listofSpawnedPosition; // hold the vector3 of spawned MapPointers. It is cleared before it collects new positions.
    ...
    private List<GameObject> SpawnedMapPointers; // hold the gameobject of spawned MapPointers. Cleared before collecting new ones.
    ...
    // To control the Moving of the floating origin in the update function
    private bool isStaticStreetNull = false;
    public bool IsInitialized = false;
    bool shouldcheckedsurroundingmarker = true;
    ...
    private IEnumerator Follow()
    {
        ...
        mapsService = GetComponent<MapsService>();
         ...
         historicStreet = SearchPanelScript.selectedLocation;

         if (historicStreet == null)
         {
             mapsService.InitFloatingOrigin(PreviousLocation);
             
             isStaticStreetNull = true;
...
/**/
     ...
     { 
         ...
         // constantly get the difference in vector between the MyLocation Marker and the Map Pointers
         
         SubtractHistoricVectors();
         
         // Only move the map location if the device has moved more than 2 meters.
        
         // Also check if the "isStaticStreetNull" is true

         if (isStaticStreetNull)
         {
    ...
    private IEnumerator GetLocation()
    {
        ...
        // Get the static List of Historic Street
        ...
        // clear the position or vector3 of the spawned MapPointers
        
        if (listofSpawnedPosition.Count > 0)
        {
            listofSpawnedPosition.Clear();
        }
    ...
    void SpawnHistoricCubes()
    {
        ...
        // This method will check if there are any marker within 1000m radius,
        // if non, then
        // it will display a popup prompting the user to add the History of the street
        // else
        // it will not do any thing

        if (shouldcheckedsurroundingmarker)
        {
            CheckSurroundingMarkers();
            shouldcheckedsurroundingmarker = false;
        }
    ...
    void CheckSurroundingMarkers()
    {
        float radius = 4000f;
        bool showPopUp = true;

        Vector3 myLocationMarker = MyLocationMaker.transform.position;

        foreach (var position in listofSpawnedPosition)
        {
            float distance = Vector3.Distance(myLocationMarker, position);
        ...
        if (showPopUp)
        {
            GameObject PopUp = Instantiate(AddStreetPopUp);
            PopUp.GetComponent<AddStreetPopUpScript>().SetUp(new LatLng(Input.location.lastData.latitude, Input.location.lastData.longitude));
